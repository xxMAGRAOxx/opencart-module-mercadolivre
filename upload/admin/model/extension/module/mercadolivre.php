...
class ModelExtensionModuleMercadolivre extends Model
{
    ...
    public function createProductInMl($product)
    {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "mercadolivre_products` SET `ml_product_code` = '" . $this->db->escape($response['body']->id) . "', `condition` = '" . $this->db->escape($response['body']->condition) . "', `product_id` = '" . $product['product_id'] . "', `listing_type_id` = '" . $this->db->escape($response['body']->listing_type_id) . "', `mercadolivre_category_id` = '" . $this->db->escape($response['body']->category_id) . "', `status` = '" . $response['body']->status . "', `subtract_product` = '" . (int) $product['subtract_product'] . "', `price_adjustments` = '" . $this->db->escape($product['price_adjustment']) . "', `created_at` = NOW()");
    ...
    {
        $request = ['listing_type_id' => $product['listing_type'], 'title' => $this->getTitle($product), 'currency_id' => $this->config->get('module_mercadolivre_currency'), 'available_quantity' => $product['quantity'], 'buying_mode' => $this->config->get('module_mercadolivre_buying_mode'), 'condition' => $this->config->get('module_mercadolivre_condition'), 'shipping' => ['mode' => !empty($this->config->get('module_mercadolivre_shipping_type')) ? $this->config->get('module_mercadolivre_shipping_type') : 'me2', 'local_pick_up' => $this->config->get('module_mercadolivre_shipping_with_local_pick_up') == 1], 'description' => ['plain_text' => $this->getDescription($product)], 'attributes' => [['id' => 'BRAND', 'value_name' => $product['manufacturer']], ['id' => 'MODEL', 'value_name' => $product['model']], ['id' => 'PACKAGE_HEIGHT', 'value_name' => intval($product['height']) . ' cm'], ['id' => 'PACKAGE_WIDTH', 'value_name' => intval($product['width']) . ' cm'], ['id' => 'PACKAGE_LENGTH', 'value_name' => intval($product['length']) . ' cm'], ['id' => 'PACKAGE_WEIGHT', 'value_name' => floatval($product['weight']) . ' kg']]];
        //$request = ['listing_type_id' => $product['listing_type'], 'title' => $this->getTitle($product), 'currency_id' => $this->config->get('module_mercadolivre_currency'), 'available_quantity' => $product['quantity'], 'buying_mode' => $this->config->get('module_mercadolivre_buying_mode'), 'condition' => $this->config->get('module_mercadolivre_condition'), 'shipping' => ['mode' => !empty($this->config->get('module_mercadolivre_shipping_type')) ? $this->config->get('module_mercadolivre_shipping_type') : 'me2', 'local_pick_up' => $this->config->get('module_mercadolivre_shipping_with_local_pick_up') == 1], 'description' => ['plain_text' => $this->getDescription($product)], 'attributes' => [['id' => 'BRAND', 'value_name' => $product['manufacturer']], ['id' => 'MODEL', 'value_name' => $product['model']], ['id' => 'EAN', 'value_name' => $product['ean']], ['id' => 'PACKAGE_HEIGHT', 'value_name' => intval($product['height']) . ' cm'], ['id' => 'PACKAGE_WIDTH', 'value_name' => intval($product['width']) . ' cm'], ['id' => 'PACKAGE_LENGTH', 'value_name' => intval($product['length']) . ' cm'], ['id' => 'PACKAGE_WEIGHT', 'value_name' => floatval($product['weight']) . ' kg']]];
        if (isset($product['shipping_free']) && $product['shipping_free']) {
            $request['shipping']['free_methods'] = [['id' => 100009, 'rule' => ['free_mode' => 'country']]];
        }
    ...
    {
        $value = (double) preg_replace('/[^0-9.]/', '', $steps[0]);
        if (strpos($steps[0], '<') !== false && $price < $value || strpos($steps[0], '>') !== false && $price > $value || strpos($steps[0], '<=') !== false && $price <= $value || strpos($steps[0], '>=') !== false && $price >= $value) {
    ...
    private function treatWarranty($warranty_type, $warranty_unit, $warranty, &$request)
    {
        if (!empty($warranty_type)) {
            $request['sale_terms'] = [['id' => 'WARRANTY_TYPE', 'value_name' => $warranty_type], ['id' => 'WARRANTY_TIME', 'value_name' => $warranty . ' ' . $warranty_unit]];
        }
    }
    ...
    private function mapVariations($variations, $price_adjustment, &$request)
    {
        ...
        foreach ($variations as $variation) {
            $variation_ml = ['attribute_combinations' => array(), 'price' => $this->getPrice($variation['price'], $price_adjustment), 'available_quantity' => $variation['quantity'], 'picture_ids' => [], 'attributes' => [['id' => 'SELLER_SKU', 'name' => 'SKU', 'value_name' => $variation['sku']]]];
            $urlMain = HTTPS_CATALOG . 'image/';
            ...
            $variation_ml['attribute_combinations'][] = ['name' => $variation['name'], 'value_name' => $variation['value_name']];
    ...
    private function prepareForRequest()
    {
        ...
        $accessToken = isset($configs[$this->key_prefix_oauth . '_access_token']) ? $configs[$this->key_prefix_oauth . '_access_token'] : null;
        $refreshToken = isset($configs[$this->key_prefix_oauth . '_refresh_token']) ? $configs[$this->key_prefix_oauth . '_refresh_token'] : null;
        $expire_at = isset($configs[$this->key_prefix_oauth . '_expire_at']) ? $configs[$this->key_prefix_oauth . '_expire_at'] : null;
        if (self::$mlSdk == null) {
    ...
    private function createRequestForUpdateStockAndPrice($product, $variations)
    {
        ...
        $price = (double) (!empty($this->config->get('module_mercadolivre_consider_special_price')) ? $this->config->get('module_mercadolivre_consider_special_price') : false) && $product_opencart['special'] ? $product_opencart['special'] : $product_opencart['price'];
        $request = [];
        if (!empty($variations)) {
            $request['variations'] = array();
            ...
            foreach ($variations as $variation) {
                ...
                $request['variations'][] = ['id' => $variation['ml_variation_code'], 'price' => $this->getPrice($priceVariantion, $product['price_adjustments']), 'available_quantity' => $option['quantity']];
            }
        } else {
    ...
    public function getProduct($product_id)
    {
        ...
        if ($query->num_rows) {
            //return array('product_id' => $query->row['product_id'], 'name' => $query->row['name'], 'description' => $query->row['description'], 'tag' => $query->row['tag'], 'model' => $query->row['model'], 'sku' => $query->row['sku'], 'ncm' => $query->row['ncm'], 'cest' => $query->row['cest'], 'upc' => $query->row['upc'], 'ean' => $query->row['ean'], 'jan' => $query->row['jan'], 'isbn' => $query->row['isbn'], 'mpn' => $query->row['mpn'], 'quantity' => $query->row['quantity'], 'shipping' => $query->row['shipping'], 'image' => $query->row['image'], 'manufacturer_id' => $query->row['manufacturer_id'], 'manufacturer' => $query->row['manufacturer'], 'price' => $query->row['discount'] ? $query->row['discount'] : $query->row['price'], 'special' => $query->row['special'], 'date_available' => $query->row['date_available'], 'weight' => $query->row['weight'], 'length' => $query->row['length'], 'width' => $query->row['width'], 'height' => $query->row['height'], 'status' => $query->row['status'], 'tax_class_id' => $query->row['tax_class_id']);
            return array('product_id' => $query->row['product_id'], 'name' => $query->row['name'], 'description' => $query->row['description'], 'tag' => $query->row['tag'], 'model' => $query->row['model'], 'sku' => $query->row['sku'], 'upc' => $query->row['upc'], 'ean' => $query->row['ean'], 'jan' => $query->row['jan'], 'isbn' => $query->row['isbn'], 'mpn' => $query->row['mpn'], 'quantity' => $query->row['quantity'], 'shipping' => $query->row['shipping'], 'image' => $query->row['image'], 'manufacturer_id' => $query->row['manufacturer_id'], 'manufacturer' => $query->row['manufacturer'], 'price' => $query->row['discount'] ? $query->row['discount'] : $query->row['price'], 'special' => $query->row['special'], 'date_available' => $query->row['date_available'], 'weight' => $query->row['weight'], 'length' => $query->row['length'], 'width' => $query->row['width'], 'height' => $query->row['height'], 'status' => $query->row['status'], 'tax_class_id' => $query->row['tax_class_id']);
        } else {
            return false;
        }
    }
    public function getProducts($data = array())
    {
        ...
        $sort_data = array('pd.name', 'p.price', 'p.quantity', 'p.status', 'p.sort_order');
    ...
    public function sendAsnwer($question_id, $message)
    {
        ...
        if (!empty($question) && empty($question['answered_at'])) {
            $this->prepareForRequest();
            $request = array('question_id' => $question['question_id'], 'text' => $message);
    ...
    private function criteriaGetProducts($data, &$sql)
    {
        if (isset($data['filter_product_connected']) && $data['filter_product_connected'] !== '') {
            if ($data['filter_product_connected'] == '0') {
                ...
                if ($data['filter_product_connected'] == '1') {
