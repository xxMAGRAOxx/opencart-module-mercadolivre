...
class ModelExtensionModuleMercadolivre extends Model
{
    ...
    private function sendMessageForBuyer($order_ml)
    {
        ...
        if (!empty($messageToBuyerEnabled) && $messageToBuyerEnabled == 'y') {
            ...
            $request = ['from' => ['user_id' => $order_ml->seller->id, 'email' => $order_ml->seller->email], 'to' => ['user_id' => $order_ml->buyer->id], 'text' => $this->config->get('module_mercadolivre_feedback_message')];
